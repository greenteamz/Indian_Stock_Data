...
master_log_filename = f"Log_Master_BSE_{current_year}_{month_str}.txt"
...
excel_filename = f"BSE_Stock_Master_All_{current_year}_{month_str}.xlsx"  # Excel file for today

# Define base directory
BASE_DIR = f"BSE_{current_year}"
...
source_worksheet = spreadsheet.worksheet('symbol_bse')  # Test sheet name
...
BQ_DATASET = f"bse_stock_{current_year}"  # Replace with your dataset name
BQ_TABLE = f"{PROJECT_ID}.{BQ_DATASET}.daily_bse_stock_{current_year}"  # Fully-qualified table name
...
def create_consolidated_csv():
    ...
    def load_consolidated_csv_to_bigquery():
        ...
        try:
            ...
            consolidated_dataset = f"consolidated_BSE_{current_year}"
            consolidated_table = f"{PROJECT_ID}.{consolidated_dataset}.Consolidated_BSE_stock_{current_year}"
...
def fetch_and_update_stock_data(symbol, total_symbol):
    try:
        ...
        log_message(f"Life count: {current_counter} Fetching data for BSE: {symbol} ...")
        ...
        if current_price != 'N/A' and previous_close != 'N/A' and previous_close != 0:
            ...
            if abs(today_growth_percentage) > 25:
                log_message(f"Today_growth_percentage: {today_growth_percentage} will be 0 for BSE: {symbol}")
        ...
        log_message(f"Error fetching data for BSE {symbol}: {e}")
        return None
...
def load_data_to_gsheet(spreadsheet):
    ...
    sheet_name = f"BSE_{current_year}"
...
log_message("BSE Daily Script execution completed.")
